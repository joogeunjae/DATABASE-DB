...
SELECT * FROM Custom; -- 모든 필드를 검색하겠다는 뜻

-- 모든 필드에 값을 넣을 때 : 테이블에 선언되어있는 순서대로 값을 넣어야한다
-- INSERT INTO 테이블명
-- VALUES(데이터1, 데이터2, ...);

INSERT INTO Custom 
VALUES(10, 'Michle', 'rewq@rewq.com', 30, 'This is Address', true);

INSERT INTO Custom
VALUES(11, 'lacls159@gmail.com', 'Jihoon', 31, 'Busan', true);

INSERT INTO Custom 
VALUES(12, 'Michle', 'rewq1@rewq.com', 30, 'This is Address', true);

# 수정문(UPDATE)
# 해당 테이블에서 특정 필드의 값을 수정 하고자 할 때 사용하는 SQL문
...
# _ : 1개의 패턴
SELECT * FROM Custom
WHERE email LIKE '%@gmail%'; # 앞뒤에 머가오든지 @gmail을 찾아라
...
# CREATE시에 참조 제약조건을 추가할 땐
# 선행적으로 참조할 테이블과 필드가 존재해야하고
# 참조할 필드가 PRIMARY KEY 혹은 UNIQUE 제약조건이 지정 되어 있어야 함
# 참조 제약조건이 걸리는 필드는 참조할 필드의 데이터 타입과 일치해야 한다
# 필드명은 상관이없다

CREATE TABLE Foreign_Talbe1(
	foreign_field INT,
...
# ALTER
CREATE TABLE Foreign_Talbe2(
	foreign_field INT
);

ALTER TABLE Foreign_Talbe2
...
# Sub Query
# 복잡한 Join문을 조금더 간결하게 사용할 수 있도록 해주는 쿼리
# SELECT, INSERT, UPDATE, DELETE, SET, DO에서 사용 가능
# FROM, WHERE 절에서 사용가능
...
# 주의 해야할 것은 인덱스가 적용되어 있는 필드를 수정 할 경우 인덱스도 함께 변경되어
# 검색 속도에 영향을 미칠 수가 있다

# CREATE INDEX 인덱스명
# ON 테이블명(필드명, ...);
CREATE INDEX seadeasu_index
ON Namgu(세대수); # 여기다가 인덱스를 걸어놨다는것은 오로지 세대수를 검색할 
				# 때만 얘기를함

# SHOW INDEX FROM 테이블명
SHOW INDEX FROM Namgu;

CREATE INDEX seadeasu_index2
ON Namgu(세대수, 인구수, 통);
...
ON HOTE.* TO 'Developer_user'@'%';

-- REVOKE
-- 사용자로부터 권한을 회수하는 쿼리문
-- REVOKE 권한1, .... ON 데이터베이스.테이블 FROM 사용자이름@호스트;
REVOKE CREATE ON HOTE.* FROM 'Developer_user'@'%';
